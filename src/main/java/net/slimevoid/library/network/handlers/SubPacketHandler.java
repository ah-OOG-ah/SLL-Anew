...
public abstract class SubPacketHandler {
    ...
    {
        executors.put(Side.CLIENT, Maps.<String, IPacketExecutor>newConcurrentMap());
        executors.put(Side.SERVER, Maps.<String, IPacketExecutor>newConcurrentMap());
    }
    ...
    public void registerClientExecutor(String commandString, IPacketExecutor executor) {
        if (executors.get(Side.CLIENT)
            .containsKey(commandString)) {
            LoggerSlimevoidLib.getInstance(Logger.filterClassName(this.toString()))
                .write(false, "Command String [" + commandString + "] already registered.", Logger.LogLevel.ERROR);
            throw new RuntimeException("Command String [" + commandString + "] already registered.");
        }
        executors.get(Side.CLIENT)
            .put(commandString, executor);
    }
    ...
    public void registerServerExecutor(String commandString, IPacketExecutor executor) {
        if (executors.get(Side.SERVER)
            .containsKey(commandString)) {
            LoggerSlimevoidLib.getInstance(Logger.filterClassName(this.toString()))
                .write(false, "Command String [" + commandString + "] already registered.", Logger.LogLevel.ERROR);
            throw new RuntimeException("Command String [" + commandString + "] already registered.");
        }
        executors.get(Side.SERVER)
            .put(commandString, executor);
    }
    ...
    protected void handleClientPacket(PacketUpdate packet, World world, EntityPlayer entityplayer) {
        ...
        if (executors.get(Side.CLIENT)
            ...
            executors.get(Side.CLIENT)
                .get(command)
                .execute(packet, world, entityplayer);
        } else {
            LoggerSlimevoidLib.getInstance(
                Logger.filterClassName(
                    this.getClass()
            ...
            throw new RuntimeException(
                "Tried to get a Packet Executor for command: " + command + " that has not been registered.");
        }
    }
