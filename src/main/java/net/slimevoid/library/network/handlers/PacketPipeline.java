...
public class PacketPipeline extends MessageToMessageCodec<FMLProxyPacket, EurysPacket> {
    ...
    public void initialize(String modChannel) {
        this.channels = NetworkRegistry.INSTANCE.newChannel(modChannel, this);
    }
    ...
    public void registerPacketHandler(int packetID, SubPacketHandler handler) {
        if (handlers.containsKey(packetID)) {
            LoggerSlimevoidLib.getInstance(Logger.filterClassName(PacketPipeline.class.toString()))
                .write(false, "PacketID [" + packetID + "] already registered.", Logger.LogLevel.ERROR);
    ...
    protected void decode(ChannelHandlerContext ctx, FMLProxyPacket msg, List<Object> out) throws Exception {
        ...
        this.getPacketHandler(discriminator)
            .onPacketData(ctx, payload.slice(), out);
    ...
    public void sendToAllAround(PacketUpdate packet, int x, int y, int z, int range, int dimension) {
        ...
        this.channels.get(Side.SERVER)
            .attr(FMLOutboundHandler.FML_MESSAGETARGETARGS)
            .set(new TargetPoint(dimension, x, y, z, range));
        this.channels.get(Side.SERVER)
            .writeAndFlush(packet);
    }
}
