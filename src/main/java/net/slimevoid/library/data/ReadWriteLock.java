...
public class ReadWriteLock {
    ...
    public synchronized void readLock(World world) throws InterruptedException {
        while (writers > 0 || writeReq > 0) {
            LoggerSlimevoidLib.getInstance("ReadWriteLock")
                .write(world.isRemote, "readLock() - waiting", LoggerSlimevoidLib.LogLevel.INFO);
            wait();
        }
        readers++;
    }
    ...
    public synchronized void writeLock(World world) throws InterruptedException {
        ...
        while (readers > 0 || writers > 0) {
            LoggerSlimevoidLib.getInstance("ReadWriteLock")
                .write(world.isRemote, "writeLock() - waiting", LoggerSlimevoidLib.LogLevel.INFO);
            wait();
        }
        writers++;
        writeReq--;
