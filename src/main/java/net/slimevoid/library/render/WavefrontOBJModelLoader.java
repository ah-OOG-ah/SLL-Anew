...
public class WavefrontOBJModelLoader {
    ...
    public ModelSlimevoidObject loadObjFile(ModelBase baseModel, int texW, int texH, String string)
        throws NumberFormatException, ArithmeticException, FaceMissingTextureException {
        if (cache.containsKey(string)) return cache.get(string);

        ModelSlimevoidObject out = new ModelSlimevoidObject(
            (new ModelRenderer(baseModel, 0, 0)).setTextureSize(texW, texH));

        String objStr = FileReader.readFile(string);

        parse(objStr, out);

        cache.put(string, out);

        sendMessage(
            "Loaded model: " + out.vertexCount()
                + " vertices, "
                + out.polyCount()
                + " polys ["
                + string/* .getName() */
    ...
    private void parse(String objStr, ModelSlimevoidObject objModel)
        throws NumberFormatException, ArithmeticException, FaceMissingTextureException {
    ...
    private void parseLine(String line, ModelSlimevoidObject objModel)
        throws NumberFormatException, ArithmeticException, FaceMissingTextureException {
        ...
        if (lineSegments.length > 0) {
            if (lineSegments[0].equals("v") && lineSegments.length >= 4) {
                float x = Float.parseFloat(
                    df.format(Float.parseFloat(lineSegments[1].trim()))
                        .replaceAll(",", "."));
                float y = Float.parseFloat(
                    df.format(Float.parseFloat(lineSegments[2].trim()))
                        .replaceAll(",", "."));
                float z = Float.parseFloat(
                    df.format(Float.parseFloat(lineSegments[3].trim()))
                        .replaceAll(",", "."));
                ...
                if (lineSegments.length >= 5) {
                    ...
                    objModel.addQuad(a, b, c, d, at, bt, ct, dt, false);
                    ...
                    objModel.addTriangle(a, b, c, at, bt, ct, false);
                }
            }
        }
    }
